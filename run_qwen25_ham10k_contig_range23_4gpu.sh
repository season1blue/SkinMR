...
RESULT_ROOT="${RESULT_ROOT:-${ROOT_DIR}/result/ablation_qwen25_ham10k_contig_range23_4gpu}"
SUMMARY_CSV="${SUMMARY_CSV:-${RESULT_ROOT}/summary.csv}"
LOG_DIR="${LOG_DIR:-${ROOT_DIR}/logs/qwen25_ham10k_contig_range23_4gpu}"
GPU_IDS=(4 5 6 7)
...
run_range() {
    ...
    target_layers="${start_layer}"
    for ((layer=start_layer + 1; layer<=end_layer; layer++)); do
        target_layers+=",${layer}"
    done
    name="layer_${start_layer}_${end_layer}"

    run_eval "${summary_path}" "${gpu_id}" "${group}" "${name}" \
        --method memvr --entropy-threshold "${ENTROPY_THRESHOLD}" --retracing-ratio "${RETRACING_RATIO}" \
        --retrace-delay-layers "${RETRACE_DELAY_LAYERS}" --retrace-target-layers "${target_layers}" \
...
run_worker_gpu4() {
    ...
    run_eval "${summary_path}" "${gpu_id}" reference no_revisit --method base
    run_eval "${summary_path}" "${gpu_id}" reference entropy_trigger_default \
    ...
    for start_layer in 0 1 2 3 4 5 6; do
        run_range "${summary_path}" "${gpu_id}" contig2 "${start_layer}" "$((start_layer + 1))"
    done
    for start_layer in 0 1 2 3 4 5; do
        run_range "${summary_path}" "${gpu_id}" contig3 "${start_layer}" "$((start_layer + 2))"
    done
}
...
run_worker_gpu5() {
    ...
    for start_layer in 7 8 9 10 11 12 13; do
        run_range "${summary_path}" "${gpu_id}" contig2 "${start_layer}" "$((start_layer + 1))"
    done
    for start_layer in 6 7 8 9 10 11; do
        run_range "${summary_path}" "${gpu_id}" contig3 "${start_layer}" "$((start_layer + 2))"
    done
}
...
run_worker_gpu6() {
    ...
    for start_layer in 14 15 16 17 18 19 20; do
        run_range "${summary_path}" "${gpu_id}" contig2 "${start_layer}" "$((start_layer + 1))"
    done
    for start_layer in 12 13 14 15 16 17; do
        run_range "${summary_path}" "${gpu_id}" contig3 "${start_layer}" "$((start_layer + 2))"
    done
}
...
run_worker_gpu7() {
    ...
    for start_layer in 21 22 23 24 25 26; do
        run_range "${summary_path}" "${gpu_id}" contig2 "${start_layer}" "$((start_layer + 1))"
    done
    for start_layer in 18 19 20 21 22 23 24 25; do
        run_range "${summary_path}" "${gpu_id}" contig3 "${start_layer}" "$((start_layer + 2))"
    done
}
...
if [[ "$FAIL" -ne 0 ]]; then
    echo "One or more contiguous range workers failed. Check ${LOG_DIR}" >&2
    exit 1
fi
...
for group_name, label in (("contig2", "best_contig2"), ("contig3", "best_contig3")):
    group_df = df[df["group"] == group_name].copy()
    if group_df.empty:
        continue
    group_df["acc"] = pd.to_numeric(group_df["acc"], errors="coerce")
    best = group_df.sort_values(["acc", "name"], ascending=[False, True]).iloc[0]
